...
pipeline {
    ...
    post {
        success {
            script {
                if (hasTargetHost()) {
                    echo "Daily report flow completed on ${remoteUser}@${params.TARGET_HOST}. Upload is handled by the monthly upload job."
        ...
        failure {
            echo "Daily report flow failed. Check Jenkins console output and remote logs under ${baseDir}/logs."
        }
    }
}
