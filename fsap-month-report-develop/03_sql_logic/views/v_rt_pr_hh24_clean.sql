CREATE OR REPLACE VIEW v_rt_pr_hh24_clean AS
WITH src AS (
  SELECT
    trim(PR_ID) AS PR_ID,
    CALTM,
  ...
  FROM read_json_auto(
    '02_source_lake/RT_PR_HH24/RT_PR_HH24-*.jsonl.gz'
  )
),
parts AS (
  SELECT
...
final AS (
  SELECT
    *,
    -- 跨年修正：若推算出的 mmdd 在 file_dt 之後，表示是上一年
    CASE
