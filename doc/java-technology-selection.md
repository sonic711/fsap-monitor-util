...
- 可固定 Gradle 版本，避免離線環境既有 Maven/Gradle 版本影響打包結果
- 可用 `bootJar` 產出單一可執行 jar
- 可在有網路環境先產出 `offline-maven-repo.zip`，再搬到離線環境用 `--offline -PofflineRepo=...` 建置
- 離線環境若已有乾淨 Gradle 8.13，可直接使用系統 Gradle；否則使用專案 Gradle Wrapper 與 `gradle-8.13-all.zip`

早期文件曾建議 Maven 作為保守選擇；v1.1 後已改以 Gradle 為現行建置方式。
...
1. `build/libs/fsap-monitor-util-0.1.0-SNAPSHOT.jar`
2. 固定版本 `JDK` 或最少 `JRE 17`
3. `config/`
4. `03_sql_logic/`
5. `offline-maven-repo.zip`，供離線環境重新打包使用
6. `gradle-8.13-all.zip`，僅在離線環境沒有可用 Gradle 8.13 且要用 wrapper 時需要
7. 啟動腳本：
   - `start.sh`
   - `start.bat`
8. 範例資料與驗證腳本
